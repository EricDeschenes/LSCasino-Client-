...
public class LineChart extends Tapet {
	...
	public void paintComponent(Graphics g) {
		...
		if (lines[4] || lines[0]) {
			...
			for (int i = 0; i < punts.size() - 1; i++) {
				g2.drawLine((int) punts.get(i).getX() + 5, (int) punts.get(i).getY() + 5,
						(int) punts.get(i + 1).getX() + 5, (int) punts.get(i + 1).getY() + 5);
			}
		...
		if (lines[4] || lines[1]) {
			...
			for (int i = 0; i < ruleta.size() - 1; i++) {
				g2.drawLine((int) ruleta.get(i).getX() + 5, (int) ruleta.get(i).getY() + 5,
						(int) ruleta.get(i + 1).getX() + 5, (int) ruleta.get(i + 1).getY() + 5);
			}
			g.setColor(Constants.coolGray);
		...
		if (lines[4] || lines[2]) {
			...
			for (int i = 0; i < cavalls.size() - 1; i++) {
				g2.drawLine((int) cavalls.get(i).getX() + 5, (int) cavalls.get(i).getY() + 5,
						(int) cavalls.get(i + 1).getX() + 5, (int) cavalls.get(i + 1).getY() + 5);
			}
			g.setColor(Constants.coolGray);
		...
		if (lines[4] || lines[3]) {
			...
			for (int i = 0; i < blackjack.size() - 1; i++) {
				g2.drawLine((int) blackjack.get(i).getX() + 5, (int) blackjack.get(i).getY() + 5,
						(int) blackjack.get(i + 1).getX() + 5, (int) blackjack.get(i + 1).getY() + 5);
			}
			g.setColor(Constants.coolGray);
	...
	public void addPoint(float x, float y, boolean goFirst, int joc) {
		...
		case 1:
			if (goFirst)
				ruleta.addFirst(punt);
			else
				ruleta.add(punt);
			break;
		case 2:
			if (goFirst)
				cavalls.addFirst(punt);
			else
				cavalls.add(punt);
			break;
		case 3:
			if (goFirst)
				blackjack.addFirst(punt);
			else
				blackjack.add(punt);
			break;
		}
